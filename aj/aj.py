...
class DeviceAPI(Resource):
    ...
    def get(self, devicename):
        ...
        logger.debug(
            "fn=DeviceAPI/get : src=%s, device=%s" % (request.remote_addr, devicename)
        )
        logaction(
            classname="DeviceAPI",
            methodname="get",
        ...
        try:
            ...
            (deviceinfo["cswMaxSwitchNum"], deviceinfo["entities"]) = self.get_serial(
                m, devicename
            )
            ...
            # sysoid mapping
            logger.trace(
                "fn=DeviceAPI/get : %s : translate_sysoid %s"
            ...
            logger.error(
                "fn=DeviceAPI/get : %s : SNMP get of generic aspects for device failed : %s"
                % (devicename, e)
            )
            return (
                errst.status(
                    "ERROR_OP",
                    "SNMP get of generic aspects failed on %s, cause : %s"
                    % (devicename, e),
                ),
                200,
            )
        ...
        try:
            ...
            logger.info(
                "fn=DeviceAPI/get : %s : SNMP get for POE aspects for device failed : %s"
                % (devicename, e)
            )
        ...
        tdiff = tend - tstart
        duration = (
            tdiff.microseconds + (tdiff.seconds + tdiff.days * 24 * 3600) * 10**6
        ) / 1000
        deviceinfo["query-duration"] = duration

        logger.info(
            "fn=DeviceAPI/get : %s : duration=%s"
    ...
    def get_serial(self, m, devicename):
                    ...
                    hardware_info.append(
                        {
                            ...
                            "physicalClass": re.sub(
                                class_regex, "", str(m.entPhysicalClass[index])
                            ),
                        }
                    )
        except snmp.SNMPException as e:
        ...
        if len(hardware_info) == 0:
            logger.warning(
                "fn=DeviceAPI/get_serial : %s : could not get an entity parent"
                % devicename
            )
        else:
            logger.debug(
                "fn=DeviceAPI/get_serial : %s : got %s serial(s)"
                % (devicename, len(hardware_info))
            )

        logger.trace(
            "fn=DeviceAPI/get_serial : %s : returning counter=%s, hardware_info=%s)"
...
class DeviceActionAPI(Resource):
    ...
    def post(self, devicename):
        ...
        logger.debug(
            "fn=DeviceActionAPI/post : src=%s, %s / %s"
            % (request.remote_addr, devicename, action)
        )
        logaction(
            classname="DeviceActionAPI",
            methodname="post",
            devicename=devicename,
            params=args,
            mode="rw",
        ...
        if action == "ping":
            ...
            logger.debug(
                "fn=DeviceActionAPI/post : %s : run action %s" % (devicename, action)
            )
            ...
            try:
                ...
                logger.debug(
                    "fn=DeviceActionAPI/post : %s : rc=<%s>, stdout=<%s>, stderr=<%s>"
                    % (devicename, rc, stdout, stderr)
                )
            except Exception as e:
                logger.error(
                    "fn=DeviceActionAPI/post : %s : ping action failed : %s"
                    % (devicename, e)
                )
                return (
                    errst.status(
                        "ERROR_OP",
                        "ping action for %s failed, cause : %s" % (devicename, e),
                    ),
                    200,
                )
        ...
        logger.info(
            "fn=DeviceActionAPI/post : %s : duration=%s"
            % (devicename, deviceinfo["query-duration"])
        )
        return deviceinfo
...
class DeviceSaveAPI(Resource):
    ...
    def put(self, devicename):
        ...
        logger.info(
            "fn=DeviceSaveAPI/put : src=%s, %s, uuid=%s"
            % (request.remote_addr, devicename, uuid)
        )
        logaction(
            classname="DeviceSaveAPI",
            methodname="put",
            devicename=devicename,
            params=args,
            mode="rw",
        ...
        try:
            ...
            logger.debug(
                "fn=DeviceSaveAPI/put : %s : operation %d : set the source to be the running-config"
                % (devicename, opidx)
            )
            m.ccCopySourceFileType[opidx] = 4
            # set the dest to be the startup-config
            logger.debug(
                "fn=DeviceSaveAPI/put : %s : operation %d : set the dest to be the startup-config"
                % (devicename, opidx)
            )
            m.ccCopyDestFileType[opidx] = 3
            # start the transfer
            logger.debug(
                "fn=DeviceSaveAPI/put : %s : operation %d : start the transfer"
                % (devicename, opidx)
            )
            ...
            while waited < write_timeout:
                ...
                if state == 3 or state == 4:
                    break
                logger.debug(
                    "fn=DeviceSaveAPI/put : %s : operation %d : waiting for config save to finish"
                    % (devicename, opidx)
                )
                time.sleep(step)

            logger.debug(
                "fn=DeviceSaveAPI/put : %s : operation %d : waited=%s seconds"
                % (devicename, opidx, waited)
            )

            if waited == write_timeout:
                logger.error(
                    "fn=DeviceSaveAPI/put : %s : operation %d : copy failed, cause = timeout"
                    % (devicename, opidx)
                )
                return (
                    errst.status(
                        "ERROR_OP",
                        "config save for %s failed, cause : timeout, operation-nr : %d"
                        % (devicename, opidx),
                    ),
                    200,
                )
            ...
            if m.ccCopyState == 4:
                ...
                logger.error(
                    "fn=DeviceSaveAPI/put : %s : operation %d : copy failed, cause = %s"
                    % (devicename, cause, opidx)
                )
                return (
                    errst.status(
                        "ERROR_OP",
                        "config save for %s failed, cause : %s, operation-nr : %s"
                        % (devicename, cause, opidx),
                    ),
                    200,
                )
                ...
                # success
                logger.info(
                    "fn=DeviceSaveAPI/put : %s : operation %d : copy successful"
                    % (devicename, opidx)
                )

            # delete op
            logger.debug(
                "fn=DeviceSaveAPI/put : %s : operation %d : clear operation"
                % (devicename, opidx)
            )
            ...
            return (
                errst.status(
                    "ERROR_OP",
                    "config save for %s failed, cause : %s" % (devicename, e),
                ),
                200,
            )
        ...
        return {
            "info": "config save for %s successful" % devicename,
            "duration": duration,
            "operation-nr": opidx,
        }
...
class InterfaceAPI(Resource):
    ...
    def get(self, devicename):
        ...
        logger.debug(
            "fn=InterfaceAPI/get : src=%s, %s" % (request.remote_addr, devicename)
        )
        ...
        logger.info(
            "fn=InterfaceAPI/get : %s : showmac=%s, showvlannames=%s, showpoe=%s, showcdp=%s, showdhcp=%s, showtrunks=%s"
            % (
                devicename,
                showmac,
                showvlannames,
        ...
        logger.debug(
            "fn=InterfaceAPI/get : %s : end poll EntityMIB, %s ports found"
        ...
        for index, desc in m.ifDescr.items():
            logger.trace(
                "fn=InterfaceAPI/get : %s : index = %s, desc = %s"
                % (devicename, index, desc)
            )
        ...
        for index, name in m.ifName.items():
            logger.trace(
                "fn=InterfaceAPI/get : %s : index = %s, name = %s"
        ...
        for index, adminstatus in m.ifAdminStatus.items():
            logger.trace(
                "fn=InterfaceAPI/get : %s : index = %s, admin-status = %s"
                % (devicename, index, adminstatus)
            )
            (
                interfaces[index]["ifAdminStatus"],
                interfaces[index]["ifAdminStatusText"],
            ) = util.translate_status(str(adminstatus))

        logger.debug("fn=InterfaceAPI/get : %s : get ifOperStatus" % devicename)
        for index, operstatus in m.ifOperStatus.items():
            logger.trace(
                "fn=InterfaceAPI/get : %s : index = %s, oper-status = %s"
                % (devicename, index, operstatus)
            )
            interfaces[index]["ifOperStatus"], interfaces[index]["ifOperStatusText"] = (
                util.translate_status(str(operstatus))
            )

        logger.debug("fn=InterfaceAPI/get : %s : get ifType" % devicename)
        for index, iftype in m.ifType.items():
            logger.trace(
                "fn=InterfaceAPI/get : %s : index = %s, iftype = %s"
        ...
        for index, ifmtu in m.ifMtu.items():
            logger.trace(
                "fn=InterfaceAPI/get : %s : index = %s, ifmtu = %s"
        ...
        for index, ifspeed in m.ifSpeed.items():
            logger.trace(
                "fn=InterfaceAPI/get : %s : index = %s, ifspeed = %s"
        ...
        for index, ifalias in m.ifAlias.items():
            logger.trace(
                "fn=InterfaceAPI/get : %s : index = %s, ifalias = %s"
        ...
        for index, duplex in m.dot3StatsDuplexStatus.items():
            logger.trace(
                "fn=InterfaceAPI/get : %s : index = %s, duplex = %s"
        ...
        for index, vlan_id in m.vmVlan.items():
            logger.trace(
                "fn=InterfaceAPI/get : %s : index = %s, vlan_id = %s"
        ...
        for interface in interfaces:
            ...
            if not "ifAdminStatus" in interfaces[interface]:
                (
                    interfaces[interface]["ifAdminStatus"],
                    interfaces[interface]["ifAdminStatusText"],
                ) = util.translate_status(str(None))

            # missing oper-status (never seen this case)
            if not "ifOperStatus" in interfaces[interface]:
                (
                    interfaces[interface]["ifOperStatus"],
                    interfaces[interface]["ifOperStatusText"],
                ) = util.translate_status(str(None))
        ...
        for index in interfaces:
            ...
            logger.debug(
                "fn=InterfaceAPI/get : %s : matching interface index=%s, desc=%s, name=%s"
            ...
            if desc in entities_if_to_chassis:
                ...
                logger.trace(
                    "fn=InterfaceAPI/get : %s : ifDescr (%s) found in physical entities"
                    % (devicename, desc)
                )
                interfaces[index]["vendorType"] = entities_if_to_chassis[desc][
                    "vendtypename"
                ]
            ...
            # 2. try ifName (works for IOS-XE)
                ...
                logger.trace(
                    "fn=InterfaceAPI/get : %s : ifName (%s) found in physical entities"
                    % (devicename, name)
                )
                interfaces[index]["vendorType"] = entities_if_to_chassis[name][
                    "vendtypename"
                ]
            ...
            # e.g. interface containers without module, virtual interfaces, trunks, etc.
                ...
                logger.trace(
                    "fn=InterfaceAPI/get : %s : ifDescr (%s) or ifName (%s) not found in physical entities"
            ...
            if showvlannames:
                ...
                logger.trace(
                    "fn=InterfaceAPI/get : %s : adding vlan names to %s"
                    % (devicename, desc)
                )
            ...
            if showmac:
                logger.trace(
                    "fn=InterfaceAPI/get : %s : adding MAC collection to %s"
                    % (devicename, desc)
                )
                if index in macs:
            ...
            if showpoe:
                logger.trace(
                    "fn=InterfaceAPI/get : %s : adding POE info to %s"
                    % (devicename, desc)
                )
                if interfaces[index]["ifDescr"] in poe:
                    ...
                    interfaces[index]["poePower"] = poe[interfaces[index]["ifDescr"]][
                        "power"
                    ]
                    ...
                    interfaces[index]["poePower"] = poe[interfaces[index]["ifName"]][
                        "power"
                    ]
                else:
            ...
            if showcdp:
                logger.trace(
                    "fn=InterfaceAPI/get : %s : adding CDP info to %s"
                    % (devicename, desc)
                )
                ...
                if index in cdps:
                    ...
                    if address_type in ("ipv4", "ipv6"):
                        interfaces[index]["cdp"]["cdpCacheAddress"] = (
                            util.convert_ip_from_snmp_format(
                                address_type, cdps[index]["cdpCacheAddress"]
                            )
                        )
                    else:
                        interfaces[index]["cdp"]["cdpCacheAddress"] = (
                            "cannot convert SNMP value for address, unsupported type %s"
                            % address_type
                        )
                    interfaces[index]["cdp"]["cdpCacheVersion"] = cdps[index][
                        "cdpCacheVersion"
                    ]
                    interfaces[index]["cdp"]["cdpCacheDeviceId"] = cdps[index][
                        "cdpCacheDeviceId"
                    ]
                    interfaces[index]["cdp"]["cdpCacheDevicePort"] = cdps[index][
                        "cdpCacheDevicePort"
                    ]
                    interfaces[index]["cdp"]["cdpCachePlatform"] = cdps[index][
                        "cdpCachePlatform"
                    ]
                    interfaces[index]["cdp"]["cdpCacheLastChange"] = cdps[index][
                        "cdpCacheLastChange"
                    ]
                else:
            ...
            if showdhcp:
                logger.trace(
                    "fn=InterfaceAPI/get : %s : adding DHCP info to %s"
                    % (devicename, desc)
                )
            ...
            if showtrunks:
                logger.trace(
                    "fn=InterfaceAPI/get : %s : adding trunks info to %s"
                    % (devicename, desc)
                )
                if index in trunks_entries:
                    interfaces[index]["trunkAdminState"] = trunks_entries[index][
                        "trunkAdminState"
                    ]
                    interfaces[index]["trunkOperState"] = trunks_entries[index][
                        "trunkOperState"
                    ]
                else:
        ...
        logger.info(
            "fn=InterfaceAPI/get : %s : found %s interfaces, duration=%s"
    ...
    def get_poe(self, devicename, m):
        ...
        logger.debug(
            "fn=InterfaceAPI/get_poe : %s : create a mapping EntPhyIndex --> port name"
            % (devicename)
        )
        ...
        for index, value in m.entPhysicalName.items():
            ...
            logger.trace(
                "fn=InterfaceAPI/get_poe : %s : port-mapping : ent-idx=%s, port-name=%s"
        ...
        try:
            ...
            logger.debug(
                "fn=InterfaceAPI/get_poe : %s : get cpeExtPsePortPwrConsumption"
                % (devicename)
            )
            ...
            logger.debug(
                "fn=InterfaceAPI/get_poe : %s : get pethPsePortDetectionStatus"
                % (devicename)
            )
            ...
            logger.debug(
                "fn=InterfaceAPI/get_poe : %s : get cpeExtPsePortEntPhyIndex"
                % (devicename)
            )
            ...
            for index in poe_parts:
                poe_parts_merged[poe_parts[index]["cpeExtPsePortEntPhyIndex"]][
                    "Consumption"
                ] = poe_parts[index]["cpeExtPsePortPwrConsumption"]
                poe_parts_merged[poe_parts[index]["cpeExtPsePortEntPhyIndex"]][
                    "Status"
                ] = poe_parts[index]["pethPsePortDetectionStatus"]
            ...
            logger.info(
                "fn=InterfaceAPI/get_poe : %s : got %s poe entries"
                % (devicename, poe_entries)
            )

        except Exception as e:
            logger.info(
                "fn=InterfaceAPI/get_poe : %s : could not get poe info, probably a device without POE. Status : %s"
                % (devicename, e)
            )
        ...
        logger.info(
            "fn=InterfaceAPI/get_poe : %s : POE collection duration=%s"
            % (devicename, duration)
        )
    ...
    def collect_entities(self, m, devicename):
        ...
        counter = 0
        logger.info(
            "fn=InterfaceAPI/collect_entities : %s : loop over entPhysicalClass"
            % devicename
        )
        for index, value in m.entPhysicalClass.items():
            logger.trace(
                "fn=InterfaceAPI/collect_entities : %s : entPhysicalClass entry %s, %s"
        ...
        logger.info(
            "fn=InterfaceAPI/collect_entities : %s : loop over entPhysicalClass done in %s, %s entries found"
        ...
        counter = 0
        logger.info(
            "fn=InterfaceAPI/collect_entities : %s : loop over entPhysicalName"
            % devicename
        )
        for index, value in m.entPhysicalName.items():
            logger.trace(
                "fn=InterfaceAPI/collect_entities : %s : entPhysicalName entry %s, %s"
                % (devicename, index, value)
            )
        ...
        logger.info(
            "fn=InterfaceAPI/collect_entities : %s : loop over entPhysicalName done in %s, %s entries found"
            % (devicename, duration, counter)
        )
        ...
        counter = 0
        logger.info(
            "fn=InterfaceAPI/collect_entities : %s : loop over entPhysicalContainedIn"
            % devicename
        )
        for index, value in m.entPhysicalContainedIn.items():
            logger.trace(
                "fn=InterfaceAPI/collect_entities : %s : entPhysicalContainedIn entry %s, %s"
                % (devicename, index, value)
            )
        ...
        logger.info(
            "fn=InterfaceAPI/collect_entities : %s : loop over entPhysicalContainedIn done in %s, %s entries found"
            % (devicename, duration, counter)
        )
        ...
        counter = 0
        logger.info(
            "fn=InterfaceAPI/collect_entities : %s : loop over entPhysicalVendorType"
            % devicename
        )
        for index, value in m.entPhysicalVendorType.items():
            logger.trace(
                "fn=InterfaceAPI/collect_entities : %s : entPhysicalVendorType entry %s, %s"
                % (devicename, index, value)
            )
        ...
        logger.info(
            "fn=InterfaceAPI/collect_entities : %s : loop over entPhysicalVendorType done in %s, %s entries found"
            % (devicename, duration, counter)
        )

        return {
            "entries_entPhysicalClass": entries_entPhysicalClass,
    ...
    def merge_entities(self, entities, devicename):
        ...
        for idx, value in entities["entries_entPhysicalClass"].items():
            logger.trace(
                "fn=InterfaceAPI/merge_entities : %s : merging index %s of class %s"
                % (devicename, idx, value)
            )
            merged_entities[idx]["class"] = value
            merged_entities[idx]["name"] = entities["entries_entPhysicalName"].get(
                idx, None
            )
            merged_entities[idx]["cin"] = entities[
                "entries_entPhysicalContainedIn"
            ].get(idx, None)
            merged_entities[idx]["vendtype"] = entities[
                "entries_entPhysicalVendorType"
            ].get(idx, None)
    ...
    def get_ports(self, merged_entities, devicename):
        ...
        for idx, entry in merged_entities.items():
            ...
            if entry["class"] == 10:
                logger.trace(
                    "fn=InterfaceAPI/get_ports : %s : searching for port %s"
                ...
                logger.trace(
                    "fn=InterfaceAPI/get_ports : %s : port %s is part of chassis %s"
                ...
                logger.debug(
                    "fn=InterfaceAPI/get_ports : %s : port %s has vendor-type %s (%s)"
    ...
    # -----------------------------------------------------------------------------------
    def find_parent_of_type(
        self, devicename, port_idx, searched_parent_type, merged_entities
    ):
        ...
        logger.trace(
            "fn=InterfaceAPI/find_parent_of_type : %s : find_parent_of_type %s for entity %s"
            % (devicename, searched_parent_type, port_idx)
        )

        parent_idx = merged_entities[port_idx]["cin"]
        logger.trace(
            "fn=InterfaceAPI/find_parent_of_type : %s : parent of port %s is %s"
        ...
        if parent_idx == 0:
            logger.info(
                "fn=InterfaceAPI/find_parent_of_type : %s : parent of port %s is zero, search stopped"
                % (devicename, port_idx)
            )
            return parent_idx
        ...
        logger.trace(
            "fn=InterfaceAPI/find_parent_of_type : %s : type of parent %s is %s"
        ...
        if type_of_parent == searched_parent_type:
            # yes !
            logger.trace(
                "fn=InterfaceAPI/find_parent_of_type : %s : parent %s has the searched type %s, search done"
...
class InterfaceCounterAPI(Resource):
    ...
    def get(self, devicename, ifindex):
        ...
        logger.debug(
            "fn=InterfaceCounterAPI/get : src=%s, %s : index=%s"
            % (request.remote_addr, devicename, ifindex)
        )
        logaction(
            classname="InterfaceCounterAPI",
            methodname="get",
            devicename=devicename,
            params=ifindex,
            src_ip=request.remote_addr,
        ...
        try:
            ...
            logger.error(
                "fn=InterfaceCounterAPI/get : %s : SNMP get failed : %s"
                % (devicename, e)
            )
            return (
                errst.status(
                    "ERROR_OP", "SNMP get failed on %s, cause : %s" % (devicename, e)
                ),
                200,
            )
        ...
        logger.info(
            "fn=InterfaceCounterAPI/get : %s : duration=%s"
            % (devicename, deviceinfo["query-duration"])
        )
        return deviceinfo
...
class MacAPI(Resource):
    ...
    def get_macs_from_device(self, devicename, m):
        ...
        logger.debug(
            "fn=MacAPI/get_macs_from_device : %s : got %s vlans"
        ...
        for vlan_nr in vlans:
            ...
            vlan_counter += 1
            logger.debug(
                "fn=MacAPI/get_macs_from_device : %s : checking vlan_nr = %s (%s of %s), name = %s, type = %s, state = %s"
                % (
                    devicename,
                    vlan_nr,
                    vlan_counter,
            ...
            if vlan_type == "ethernet(1)" and vlan_state == "operational(1)":
                logger.debug(
                    "fn=MacAPI/get_macs_from_device : %s : polling vlan %s (%s)"
                ...
                logger.debug(
                    "fn=MacAPI/get_macs_from_device : %s : requesting a SNMP manager"
                    % (devicename)
                )
                ...
                try:
                    logger.debug(
                        "fn=MacAPI/get_macs_from_device : %s : trying to pull all mac_entries for vlan %s (%s)"
                        % (devicename, vlan_nr, vlan_name)
                    )
                    ...
                    for index, mac_entry in lm.dot1dTpFdbAddress.items():
                        ...
                        mac_entries += 1
                    logger.debug(
                        "fn=MacAPI/get_macs_from_device : %s : got %s dot1dTpFdbAddress entries for vlan %s (%s)"
                    ...
                    if mac_entries > 0:
                        ...
                        logger.debug(
                            "fn=MacAPI/get_macs_from_device : %s : got %s dot1dTpFdbPort entries for vlan %s (%s)"
                        ...
                        logger.debug(
                            "fn=MacAPI/get_macs_from_device : %s : got %s dot1dBasePortIfIndex entries for vlan %s (%s)"
                            % (
                                devicename,
                                len(dot1dBasePortIfIndex),
                                vlan_nr,
                                vlan_name,
                            )
                        )
                        ...
                        logger.debug(
                            "fn=MacAPI/get_macs_from_device : %s : enrich MAC table for vlan %s (%s)"
                            % (devicename, vlan_nr, vlan_name)
                        )
                        for mac_entry in dot1dTpFdbAddress:
                            ...
                            if port == None:
                                logger.debug(
                                    "fn=MacAPI/get_macs_from_device : %s vlan %s : skip port=None"
                                    % (devicename, vlan_nr)
                                )
                                continue
                            ...
                            try:
                                ...
                                logger.debug(
                                    "fn=MacAPI/get_macs_from_device : %s : port=%s, mac_entry_idx lookup failed : %s"
                                    % (devicename, port, e)
                                )
                            ...
                            try:
                                ...
                                logger.trace(
                                    "fn=MacAPI/get_macs_from_device : %s : vendor lookup failed : %s"
                                    % (devicename, e)
                                )
                                vendor = "unknown"

                            logger.trace(
                                "idx=%s, vlan=%s, mac=%s, vendor=%s"
                        ...
                        logger.debug(
                            "fn=MacAPI/get_macs_from_device : %s : vlan %s (%s) skipped, no MAC found on it"
                            % (devicename, vlan_nr, vlan_name)
                        )

                except:
                    logger.info(
                        "fn=MacAPI/get_macs_from_device : %s : failed, probably an unused VLAN (%s) on a buggy IOS producing SNMP timeout. Ignoring this VLAN"
                        % (devicename, vlan_nr)
                    )

            else:
                logger.debug(
                    "fn=MacAPI/get_macs_from_device : %s : skipping vlan %s (%s)"
                    % (devicename, vlan_nr, vlan_name)
                )

            logger.debug(
                "fn=MacAPI/get_macs_from_device : %s : %s mac entries found in vlan %s (%s)"
                % (devicename, mac_entries, vlan_nr, vlan_name)
            )
            total_mac_entries += mac_entries

        logger.debug(
            "fn=MacAPI/get_macs_from_device : %s : returning data, total %s mac entries found"
...
class CDPAPI(Resource):
    ...
    def get_cdp_from_device(self, devicename, m):
        ...
        try:
            logger.debug(
                "fn=CDPAPI/get_cdp_from_device : %s : get cdpCacheAddressType"
                % devicename
            )
            for index, value in m.cdpCacheAddressType.items():
                ...
                if str(value) == "ip(1)":
                    ...
                    logger.warning(
                        "fn=CDPAPI/get_cdp_from_device : %s : unsupported cdpCacheAddressType <%s>"
                        % (devicename, value)
                    )
            ...
            logger.debug(
                "fn=CDPAPI/get_cdp_from_device : %s : get cdpCacheDevicePort"
                % devicename
            )
            ...
            logger.debug(
                "fn=CDPAPI/get_cdp_from_device : %s : get cdpCacheLastChange"
                % devicename
            )
...
class TrunkAPI(Resource):
    ...
    def get(self, devicename):
        ...
        try:
            ...
            logger.error(
                "fn=TrunkAPI/get : %s : SNMP get failed : %s" % (devicename, e)
            )
            return (
                errst.status(
                    "ERROR_OP", "SNMP get failed on %s, cause : %s" % (devicename, e)
    ...
    def get_trunks_from_device(self, devicename, m):
        ...
        try:
            ...
            for index, value in m.vlanTrunkPortDynamicState.items():
                logger.trace(
                    "fn=TrunkAPI/get_trunks_from_device/1 : trunk : %s, %s"
                    % (index, value)
                )
            ...
            for index, value in m.vlanTrunkPortDynamicStatus.items():
                logger.trace(
                    "fn=TrunkAPI/get_trunks_from_device/2 : trunk : %s, %s"
                    % (index, value)
                )
            ...
            logger.warning(
                "fn=TrunkAPI/get_trunks_from_device : failed SNMP get for Trunks : %s"
                % e
            )

        logger.debug(
            "fn=TrunkAPI/get_trunks_from_device : returning data : %s trunks entries found"
            % len(trunks)
        )
        return trunks
    ...
    def get_arp_from_device(self, devicename, m):
        ...
        try:
            ...
            for index, value in m.ipNetToPhysicalPhysAddress.items():
                ...
                i += 1
                logger.trace(
                    "fn=ARPAPI/get_arp_from_device : %s : idx=%s, mac=%s, ip=%s"
                    % (devicename, entry["ifindex"], entry["mac"], entry["ip"])
                )

            nbr_of_entries = i
            logger.info(
                "fn=ARPAPI/get_arp_from_device : %s : got %s ARP entries"
            ...
            if str(e) == "no more stuff after this OID":
                logger.info(
                    "fn=ARPAPI/get_arp_from_device : %s : empty results, probably unsupported OID: %s"
                    % (devicename, e)
                )
            else:
                logger.warning(
        ...
        logger.debug(
            "fn=ARPAPI/get_arp_from_device %s retry using deprecated OID (ipNetToMediaPhysAddress)"
            % devicename
        )
        try:
            i = 0
            ...
            for index, value in m.ipNetToMediaPhysAddress.items():
                ...
                entry["mac"] = str(
                    netaddr.EUI(netaddr.strategy.eui48.packed_to_int(value))
                )
...
class DHCPsnoopAPI(Resource):
    ...
    def get(self, devicename):
        logger.debug(
            "fn=DHCPsnoopAPI/get : src=%s, %s" % (request.remote_addr, devicename)
        )
        logaction(
            classname="DHCPsnoopAPI",
            methodname="get",
            devicename=devicename,
            src_ip=request.remote_addr,
            src_user=auth.username(),
    ...
    def get_dhcp_snooping_from_device(self, devicename, m):
        ...
        try:
            ...
            logger.debug(
                "fn=DHCPsnoopAPI/get_dhcp_snooping_from_device : %s : get cdsBindingsAddrType"
                % devicename
            )
            cdsBindingsAddrType = {}
            ...
            logger.trace(
                "fn=DHCPsnoopAPI/get_dhcp_snooping_from_device : %s : got %s entries"
                % (devicename, len(cdsBindingsAddrType))
            )

            logger.debug(
                "fn=DHCPsnoopAPI/get_dhcp_snooping_from_device : %s : get cdsBindingsIpAddress"
                % devicename
            )
            ...
            logger.debug(
                "fn=DHCPsnoopAPI/get_dhcp_snooping_from_device : %s : get cdsBindingsInterface"
                % devicename
            )
            ...
            logger.debug(
                "fn=DHCPsnoopAPI/get_dhcp_snooping_from_device : %s : get cdsBindingsLeasedTime"
                % devicename
            )
            ...
            logger.debug(
                "fn=DHCPsnoopAPI/get_dhcp_snooping_from_device : %s : get cdsBindingsStatus"
                % devicename
            )
            cdsBindingsStatus = {}
            ...
            logger.debug(
                "fn=DHCPsnoopAPI/get_dhcp_snooping_from_device : %s : get cdsBindingsHostname"
                % devicename
            )
            cdsBindingsHostname = {}
            ...
            for index in cdsBindingsAddrType:
                ...
                try:
                    ...
                    logger.warning(
                        "fn=DHCPsnoopAPI/get_dhcp_snooping_from_device: %s : error %s : unknown vendor for %s"
                        % (devicename, e, mac_f)
                    )
                    vendor = "unknown"
                ...
                logger.trace(
                    "fn=DHCPsnoopAPI/get_dhcp_snooping_from_device %s : vlan=%s, mac=%s, vendor=%s, address_type=%s, ip=%s, interface_idx=%s, leased_time=%s, status=%s, hostname=%s"
                    % (
                        devicename,
                        vlan,
                        mac_f,
            ...
            logger.warning(
                "fn=DHCPsnoopAPI/get_dhcp_snooping_from_device : failed SNMP get for DHCP snooping : %s"
                % e
            )

        logger.debug(
            "fn=DHCPsnoopAPI/get_dhcp_snooping_from_device : returning data : %s entries found"
...
class vlanlistAPI(Resource):
    ...
    def get(self, devicename):
        ...
        logger.debug(
            "fn=vlanlistAPI/get : src=%s, device=%s" % (request.remote_addr, devicename)
        )
        logaction(
            classname="vlanlistAPI",
            methodname="get",
            devicename=devicename,
            src_ip=request.remote_addr,
            src_user=auth.username(),
        ...
        try:
            ...
            logger.debug("fn=vlanlistAPI/get : %s : get data vlan list" % devicename)
        ...
        logger.info(
            "fn=vlanlistAPI/get : %s : duration=%s"
            % (devicename, deviceinfo["query-duration"])
        )
        return deviceinfo
    ...
    def get_voice_vlans(self, devicename, m):
        ...
        try:
            ...
            for index, value in m.vmVoiceVlanId.items():
                logger.trace(
                    "fn=vlanlistAPI/get_voice_vlans : %s : got voice vlan %s for index %s"
            ...
            logger.info(
                "fn=vlanlistAPI/get_voice_vlans : %s : SNMP get failed : %s"
                % (devicename, e)
            )
...
class PortToVlanAPI(Resource):
    ...
    def put(self, devicename, ifindex):
        ...
        logger.info(
            "fn=PortToVlanAPI/put : src=%s, %s : ifindex=%s, vlan=%s, uuid=%s"
            % (request.remote_addr, devicename, ifindex, vlan, uuid)
        )
        logaction(
            classname="PortToVlanAPI",
            methodname="put",
            devicename=devicename,
            params=args,
            mode="rw",
        ...
        logger.debug(
            "fn=PortToVlanAPI/put : %s : VLAN %s assigned to interface-idx %s successfully"
            % (devicename, vlan, ifindex)
        )

        return {
            "info": "%s : VLAN %s assigned to interface-idx %s successfully"
            % (devicename, vlan, ifindex),
            "duration": duration,
        }
...
class InterfaceConfigAPI(Resource):
    ...
    def put(self, devicename, ifindex):
        ...
        logger.info(
            "fn=InterfaceConfigAPI/put : src=%s,, %s : ifindex=%s, "
            "ifAdminStatus=%s, ifAlias=%s, uuid=%s"
            % (request.remote_addr, devicename, ifindex, ifAdminStatus, ifAlias, uuid)
        )
        logaction(
            classname="InterfaceConfigAPI",
            methodname="put",
            devicename=devicename,
            params=args,
            mode="rw",
        ...
        try:
            ...
            logger.error(
                "fn=InterfaceConfigAPI/put : %s : interface configuration failed : %s"
                % (devicename, e)
            )
            return (
                errst.status("ERROR_OP", "interface configuration failed : %s" % e),
                200,
            )
        ...
        logger.debug(
            "fn=InterfaceConfigAPI/put : %s : interface configured successfully"
            % devicename
        )
...
class OIDpumpAPI(Resource):
    ...
    def get(self, devicename, pdu, oid):
        ...
        logger.debug(
            "fn=OIDpumpAPI/get : src=%s, %s : pdu=%s, oid=%s"
            % (request.remote_addr, devicename, pdu, oid)
        )
        logaction(
            classname="OIDpumpAPI",
            methodname="get",
            devicename=devicename,
        ...
        if pdu == "get":
            try:
                logger.debug(
                    "fn=OIDpumpAPI/get : %s : SNMP get on %s" % (devicename, oid)
                )
        ...
        for entry in data:
            ...
            # and we try to detect a \x "marker"
                ...
                try:
                    ...
                    logger.warning(
                        "fn=OIDpumpAPI/get : %s : %s %s : bytes decoding failed : %s"
                        % (devicename, pdu, oid, e)
                    )
                    value = entry[1]
            else:
        ...
        logger.info(
            "fn=OIDpumpAPI/get : %s : duration=%s"
            % (devicename, deviceinfo["query-duration"])
        )
        return deviceinfo
...
class DeviceSshAPI(Resource):
    ...
    def put(self, devicename):
        ...
        driver = args["driver"]
        logger.debug(
            "fn=DeviceSshAPI/put : src=%s, Received CmdList = <%s>"
        ...
        try:
            ...
            logger.error(
                "fn=DeviceSshAPI/put : %s : %s : device configuration failed : cmds list is no valid JSON. Received CmdList = <%s>"
                % (devicename, e, args["CmdList"])
            )
            return (
                errst.status(
                    "ERROR_OP",
                    'device configuration failed : cmds list is no valid JSON : %s. Try with something like this without the backslashes : ["terminal length 0", "show users", "show version"]'
                    % e,
                ),
                500,
            )
        ...
        logger.info(
            "fn=DeviceSshAPI/put : %s : commands=%s, uuid=%s"
            % (devicename, cmdlist, uuid)
        )
        logaction(
            classname="DeviceSshAPI",
            methodname="put",
            devicename=devicename,
            params=args,
            mode="rw",
        ...
        if status == 0:
            logger.debug(
                "fn=DeviceSshAPI/put : %s : status = %s, output_indexed=%s, output_global = %s"
                % (devicename, status, output_indexed, output_global)
            )
        else:
            logger.error(
                "fn=DeviceSshAPI/put : %s : status = %s, output_indexed=%s, output global = %s"
                % (devicename, status, output_indexed, output_global)
            )
            return (
                errst.status(
                    "ERROR_OP",
                    "device commands by ssh failed : status=%s, output_indexed=%s, output_global=%s"
                    % (status, output_indexed, output_global),
                ),
                200,
            )
        ...
        return {
            "info": "device commands successful",
            "duration": duration,
...
actionhdlr = logging.handlers.RotatingFileHandler(
    action_log_file,
    maxBytes=app.config["LOG_MAX_SIZE"],
    backupCount=app.config["LOG_BACKUP_COUNT"],
)
...
doc.add(loads(DeviceAPI.__doc__), "/aj/api/v1/device/<string:devicename>", ["GET"])

api.add_resource(DeviceActionAPI, "/aj/api/v1/device/<string:devicename>/action")
doc.add(
    loads(DeviceActionAPI.__doc__),
...
doc.add(
    loads(DeviceSaveAPI.__doc__), "/aj/api/v1/devicesave/<string:devicename>", ["PUT"]
)

api.add_resource(InterfaceAPI, "/aj/api/v1/interfaces/<string:devicename>")
doc.add(
    loads(InterfaceAPI.__doc__), "/aj/api/v1/interfaces/<string:devicename>", ["GET"]
)
...
doc.add(
    loads(InterfaceCounterAPI.__doc__),
    "/aj/api/v1/interface/counter/<string:devicename>/<string:ifindex>",
    ["GET"],
)

api.add_resource(MacAPI, "/aj/api/v1/macs/<string:devicename>")
doc.add(loads(MacAPI.__doc__), "/aj/api/v1/macs/<string:devicename>", ["GET"])

api.add_resource(DHCPsnoopAPI, "/aj/api/v1/dhcpsnoop/<string:devicename>")
doc.add(
    loads(DHCPsnoopAPI.__doc__), "/aj/api/v1/dhcpsnoop/<string:devicename>", ["GET"]
)

api.add_resource(vlanlistAPI, "/aj/api/v1/vlans/<string:devicename>")
doc.add(loads(vlanlistAPI.__doc__), "/aj/api/v1/vlans/<string:devicename>", ["GET"])

api.add_resource(PortToVlanAPI, "/aj/api/v1/vlan/<string:devicename>/<string:ifindex>")
doc.add(
    loads(PortToVlanAPI.__doc__),
...
doc.add(
    loads(InterfaceConfigAPI.__doc__),
    "/aj/api/v1/interface/config/<string:devicename>/<string:ifindex>",
    ["PUT"],
)

api.add_resource(
    OIDpumpAPI, "/aj/api/v1/oidpump/<string:devicename>/<string:pdu>/<string:oid>"
)
doc.add(
    loads(OIDpumpAPI.__doc__),
...
doc.add(
    loads(DeviceSshAPI.__doc__), "/aj/api/v1/device/ssh/<string:devicename>", ["PUT"]
)

api.add_resource(CDPAPI, "/aj/api/v1/cdp/<string:devicename>")
doc.add(loads(CDPAPI.__doc__), "/aj/api/v1/cdp/<string:devicename>", ["GET"])

api.add_resource(TrunkAPI, "/aj/api/v1/trunk/<string:devicename>")
doc.add(loads(TrunkAPI.__doc__), "/aj/api/v1/trunk/<string:devicename>", ["GET"])

api.add_resource(ARPAPI, "/aj/api/v1/arp/<string:devicename>")
doc.add(loads(ARPAPI.__doc__), "/aj/api/v1/arp/<string:devicename>", ["GET"])

api.add_resource(QoSAPI, "/aj/api/v1/qos/<string:devicename>")
doc.add(loads(QoSAPI.__doc__), "/aj/api/v1/qos/<string:devicename>", ["GET"])
...
doc.add(
    loads(
        '{"name": "DocAPI", "description": "GET / : API documentation", "auth": false, "auth-type": "", "params": [], "returns": "API documentation"}'
    ),
    "/",
    ["GET"],
)
...
def logaction(
    ...
    actionlogger.info(
        "%s/%s : dev=%s params=%s mode=%s ip=%s srvuser=%s"
